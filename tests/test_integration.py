...
def crop_grid() -> CropOccupancyGrid:
    ...
    data = np.zeros((20, 20))
    data[:, 14:] = 1.0  # crop on the right half
    return CropOccupancyGrid(data=data, resolution=0.5, origin_x=0.0, origin_y=0.0)


@pytest.fixture()
def vehicle() -> VehicleState:
    return VehicleState(x=5.0, y=5.0, v_current=3.0, heading=0.0)


@pytest.fixture()
def sgg_service() -> SGGService:
    return SGGService(SGGConfig())


@pytest.fixture()
def apf_service() -> APFService:
    return APFService(APFConfig())


class TestIntegration:
    def test_no_obstacles_follows_contour(
        self, apf_service: APFService, crop_grid: CropOccupancyGrid,
        vehicle: VehicleState, sgg_service: SGGService,
    ) -> None:
    ...
    def test_obstacle_ahead_causes_braking(
        self, apf_service: APFService, crop_grid: CropOccupancyGrid,
        vehicle: VehicleState, sgg_service: SGGService,
    ) -> None:
    ...
    def test_hard_brake_for_imminent_human(
        self, apf_service: APFService, crop_grid: CropOccupancyGrid,
        vehicle: VehicleState, sgg_service: SGGService,
    ) -> None:
    ...
    def test_output_within_physical_limits(
        self, apf_service: APFService, crop_grid: CropOccupancyGrid,
        vehicle: VehicleState, sgg_service: SGGService,
    ) -> None:
        ...
        kins = [
            ...
            KinematicsEntity(id=2, cls="person", x=-1.0, y=8.0, vx=0.0, vy=-0.3),
        ]
        sems = [
