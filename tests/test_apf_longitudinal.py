...
class TestBuildSafetyCorridor:
    def test_corridor_is_valid_polygon(self) -> None:
        poly = build_safety_corridor(
            v_current=3.0, theta_max=0.5, machine_width=3.0,
            corridor_length_factor=2.0,
        )
        assert poly.is_valid
        assert poly.area > 0

    def test_corridor_length_scales_with_velocity(self) -> None:
        slow = build_safety_corridor(1.0, 0.3, 3.0, 2.0)
        fast = build_safety_corridor(5.0, 0.3, 3.0, 2.0)
        # Fast corridor should be longer → larger area
        assert fast.area > slow.area

    def test_zero_velocity_gets_minimum_corridor(self) -> None:
        poly = build_safety_corridor(0.0, 0.5, 3.0, 2.0)
        assert poly.is_valid
        assert poly.area > 0  # min length = 0.5 m
...
class TestInCorridor:
    def test_point_directly_ahead(self) -> None:
        poly = build_safety_corridor(3.0, 0.5, 3.0, 2.0)
        assert in_corridor(0.0, 1.0, poly) is True

    def test_point_behind(self) -> None:
        poly = build_safety_corridor(3.0, 0.5, 3.0, 2.0)
        assert in_corridor(0.0, -1.0, poly) is False

    def test_point_far_lateral(self) -> None:
        poly = build_safety_corridor(3.0, 0.5, 3.0, 2.0)
...
class TestComputeTargetVelocity:
    ...
    def test_entity_not_in_corridor(self) -> None:
        entities = [(10.0, 5.0, 0.9, 0.9, False)]
        v = compute_target_velocity(entities, v_base=3.0, v_max=5.0)
        assert v == pytest.approx(3.0)

    def test_entity_in_corridor_slows_down(self) -> None:
        # c=0.9, q=0.9, y_pred=3.0 → threat=0.27 → v=3*(1-0.27)=2.19
        entities = [(0.0, 3.0, 0.9, 0.9, True)]
        v = compute_target_velocity(entities, v_base=3.0, v_max=5.0)
        assert v == pytest.approx(3.0 * (1.0 - 0.27))

    def test_hard_brake_for_high_threat(self) -> None:
        # c=1.0, q=1.0, y_pred=0.5 → threat=2.0 → clamped to 1 → v=0
        entities = [(0.0, 0.5, 1.0, 1.0, True)]
        v = compute_target_velocity(entities, v_base=3.0, v_max=5.0)
        assert v == pytest.approx(0.0)

    def test_behind_entity_ignored(self) -> None:
        entities = [(0.0, -1.0, 1.0, 1.0, True)]
        v = compute_target_velocity(entities, v_base=3.0, v_max=5.0)
        assert v == pytest.approx(3.0)

    def test_velocity_clamped_to_vmax(self) -> None:
